...
export const constants = { MILIMETER_PER_STEP }
...
const SC_VALUES = z.discriminatedUnion("parameter", [
  ...
  z.object({ parameter: z.literal(SC.RC_SERVO_MIN), integer: z.number().min(1).max(65535).default(12000)
  ...
  z.object({ parameter: z.literal(SC.RC_SERVO_MAX), integer: z.number().min(1).max(65535).default(16000)
  ...
  z.object({ parameter: z.literal(SC.MAXIMUM_S2_CHANNELS), integer: z.number().min(1).max(24).default(8)
  ...
  z.object({ parameter: z.literal(SC.S2_CHANNEL_DURATION_MS), integer: z.number().min(1).max(6).default(3)
    .describe('Set the number of milliseconds before firing the next enabled channel for the S2 command.')}),
  z.object({ parameter: z.literal(SC.SERVO_RATE), integer: z.number().min(0).max(65535)
  ...
  z.object({ parameter: z.literal(SC.SERVO_RATE_UP), integer: z.number().min(0).max(65535)
    .describe('Set the rate of change of the servo when going up. Same units as rate parameter in S2 command.')}),
  z.object({ parameter: z.literal(SC.SERVO_RATE_DOWN), integer: z.number().min(0).max(65535)
